...
class MinimalDrawback:
    ...
    # Additional data like affected pieces, threat squares
...
class Storage:
        ...
        with self.db.get_session() as session:
            ...
            minimal_drawbacks = [
                MinimalDrawback(
                    ...
                    metadata={k: v for k, v in drawback.get_legal_moves_response(
                    ).items() if k != "legal_moves"}
                )
    ...
    def get_training_positions(self, limit: int = 10000,
        ...
        Yields tuples of (fen, legal_moves, drawback_info, result, move_history, prev_fen)
        """
        with self.db.get_session() as session:
            ...
            for position, result in query.all():
                ...
                if include_history:
                    ...
                    # Get the FEN of the state just before this one
                    prev_pos = session.query(Position).filter(
                        Position.game_id == position.game_id,
                        Position.move_number == position.move_number - 1
                    ).first()
                    if prev_pos:
                        prev_fen = prev_pos.fen

                yield (position.fen, position.get_legal_moves(), drawback_info, result, history, prev_fen)
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self.db.get_session() as session:
            ...
            stats['white_wins'] = session.query(Game).filter(
                Game.result == 'white_win').count()
            stats['black_wins'] = session.query(Game).filter(
                Game.result == 'black_win').count()
            stats['draws'] = session.query(Game).filter(
                Game.result == 'draw').count()
    ...
    def cleanup_old_games(self, keep_count: int = 10000):
        ...
        with self.db.get_session() as session:
            ...
            # Delete old games
            session.query(Game).filter(~Game.id.in_(keep_ids)
